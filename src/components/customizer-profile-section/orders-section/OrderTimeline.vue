  ...
  <div class="p-4 space-y-6">
    <div class="flex flex-col gap-4">
      <div v-for="(item, idx) in order.items || []" :key="`item-${idx}`" class="border rounded p-3">
        <div class="font-semibold text-muted-foreground mb-2">Factory {{ idx + 1 }}</div>
        <div
        ...
        <div class="mt-3 space-y-3">
          ...
          >
            ...
            <div class="text-xs text-muted-foreground">{{ act.time }}</div>
            <div v-if="act.message" class="text-sm text-muted-foreground mt-1">
              {{ act.message }}
            </div>
          </div>
          <div v-if="!getActivities(item).length" class="text-sm text-muted-foreground">
            No timeline entries.
          </div>
        </div>
      </div>
